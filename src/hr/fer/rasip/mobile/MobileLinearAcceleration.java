...
public class MobileLinearAcceleration extends AbstractVirtualSensor{

	private final transient Logger logger = Logger.getLogger(this.getClass());
	
	private static final String DEFAULT_DEVICE_ID_DELIMITER = "android_id";
	private static final String DEFAULT_DELIMITER = "LINEAR_ACC";
	...
	public void dataAvailable(String inputStreamName,
		...
		linearAccX = Double.parseDouble(pieces[0].replace(',', '.'));
		linearAccY = Double.parseDouble(pieces[1].replace(',', '.'));
		linearAccZ = Double.parseDouble(pieces[2].replace(',', '.'));
		
		dataProduced(new StreamElement(outputStructure, new Serializable[] { linearAccX, linearAccY, linearAccZ }, streamElement.getTimeStamp()));
	}
}
