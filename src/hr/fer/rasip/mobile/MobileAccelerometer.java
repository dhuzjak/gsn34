...
public class MobileAccelerometer extends AbstractVirtualSensor {

	private final transient Logger logger = Logger.getLogger(this.getClass());
	
	private static final String DEFAULT_DEVICE_ID_DELIMITER = "android_id";
	private static final String DEFAULT_DELIMITER = "ACCELERATION";
	...
	public void dataAvailable(String inputStreamName,
		...
		accelerometerX = Double.parseDouble(pieces[0].replace(',', '.'));
		accelerometerY = Double.parseDouble(pieces[1].replace(',', '.'));
		accelerometerZ = Double.parseDouble(pieces[2].replace(',', '.'));
		
		dataProduced(new StreamElement(outputStructure, new Serializable[] { accelerometerX, accelerometerY, accelerometerZ }, streamElement.getTimeStamp()));
	}
}
